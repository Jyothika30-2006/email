...
| `gmail_bec_subtle.eml` | **Gmail webmail sender**: only Google relay IPs (`client-ip=74.125.20.46` is Google's own edge) → personal IP unrecoverable; Message-ID `@mail.gmail.com`, `+0530` date offset, Reply-To domain mismatch, lure-domain trace, polite-but-archaic style | **SUSPICIOUS** · risk 33.1 · confidence 76.1 % · `exit 1` · `origin=UNRECOVERABLE kind=phishing_infrastructure ceiling=62%` (the *lure* IP 203.0.113.88 is reported instead, explicitly labelled as attacker infrastructure) · no fake pin · writes `reply_draft.html` (fallback (d), pixel embedded unless `--no-pixel`) |
